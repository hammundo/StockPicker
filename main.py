...
def main(name) -> None:
    ...
    def fetch_ticker(ticker: str, time_range: int, interval: int) -> pd.DataFrame:
    ...
    def create_level_coords(data: pd.DataFrame,):
        ...
        x_y_levels = {'Support X': support_x, 'Support Y': new_sup_y, 'Resistance X': resistance_x, 'Resistance Y': new_res_y}

        # Create return df
        levels = pd.DataFrame(data=x_y_levels)
        return levels
